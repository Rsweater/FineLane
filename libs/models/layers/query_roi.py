...
class AnchorVecFeatureMapAttention(nn.Module):
    ...
    def forward(self, query, roi):
        """
        Forward function for cross attention.
        Args:
            roi (torch.Tensor): Features pooled by priors, shape (B, C, Np).
        ...
        key = self.f_key(roi)  # [B, C, Np]
        value = self.f_value(roi)  # [B, C, Np]
        value = value.permute(0, 2, 1)  # [B, Np, C]

        # attention: [B, Np, C] x [B, C, Np] -> [B, Np, Np]
