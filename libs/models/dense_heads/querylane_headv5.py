...
class QueryLaneHeadV5(nn.Module):
    def __init__(
        self,
        img_w=800,
        img_h=320,
        ...
        super(QueryLaneHeadV5, self).__init__()
        ...
        self.feat_sample_points = feat_sample_points
        self.fc_hidden_dim = fc_hidden_dim
        self.prior_feat_channels = prior_feat_channels
        self.loss_sample_points = loss_sample_points
        attention.num_priors = num_priors
    ...
    def forward(self, x, **kwargs):
        ...
        Args:
            feature_pyramid: Input features (list[Tensor]). Each tensor has a shape (B, T, C, H_i, W_i),
        ...
        time_step = x[0].shape[1]
        feature_pyramid = list(x[len(x) - self.refine_layers :])
        feature_pyramid.reverse()  # (B, T, 64, 10, 25) (B, T, 64, 20, 50) (B, T, 64, 40, 100)
        proposal_feature = feature_pyramid[0][:, -1, ...]  # (B, 64, 10, 25)
        prefusion_feature = feature_pyramid[1:]  # (B, T, 64, 20, 50) (B, T, 64, 40, 100)
        # [(B, T, 64, 20, 50) (B, T, 64, 40, 100)] -> [
        # (B, T1, 64, 20, 50) (B, T2, 64, 20, 50) (B, T3, 64, 20, 50) 
        # (B, T1, 64, 40, 100)(B, T2, 64, 40, 100)(B, T3, 64, 40, 100)
        # ]
        prefusion_feature_0 = torch.chunk(feature_pyramid[0], chunks=time_step, dim=1)
        prefusion_feature_1 = torch.chunk(feature_pyramid[1], chunks=time_step, dim=1)
        # 将拆分后的 tensor 重新组合
        prefusion_feature = []
        for i in range(time_step):
            prefusion_feature.append(prefusion_feature_0[i])  # 添加第一个tensor的拆分部分
            prefusion_feature.append(prefusion_feature_1[i])  # 添加第二个tensor的拆分部分
        ...
        # pooled_features = []
        for _, feature in enumerate(prefusion_feature):
    ...
    def forward_train(self, x, img_metas, **kwargs):
        ...
        if self.loss_seg:
            out_dict["seg"] = self.forward_seg(x[:,-1,...])
