...
class SPGLaneHead(nn.Module):
    def __init__(
        ...
        super(SPGLaneHead, self).__init__()
        ...
        self.pro_reg_layers = nn.Conv1d(prior_feat_channels, self.n_offsets + 4, 1)
    ...
    def pool_prior_features(self, batch_features, prior_xs):
        ...
        feature = feature.reshape(
            # batch_size*self.num_priors, -1, self.sample_points
    ...
    def forward(self, x, **kwargs):
        ...
        pro_cls = self.pro_cls_layers(pro_fc_feature.clone()).permute(0, 2, 1).contiguous() # (B, Np, 2)

        # reference points initialization
        init_anchor_xyt = self.anchor_generator.prior_embeddings.weight.clone().repeat(
            batch_size, 1, 1
        )  # [B, Np, 3]
        # reg processing: generate reference lanes
        anchor_xyt = init_anchor_xyt + pro_reg[:, :, :3]  # y0, x0, theta
        # anchor_xyt = pro_reg[:, :, :3]  # y0, x0, theta
        updated_proposal_xs, _ = self.anchor_generator.generate_anchors(
            anchor_xyt.view(-1, 3),
            self.prior_ys,
        ...
        anchor_xyt = anchor_xyt.detach().clone()
        priors_on_featmap = pro_reg_xs.detach().clone()[
            ..., self.sample_x_indices
        ]
        pooled_features_stages = []
        for feature in prefusion_feature:
            # [B, C, H, W] X [B, Np, Ns] => [B * Np, C, Ns]
        ...
        # 4. reg processing
        xyt = anchor_xyt + reg[:, :, :3]  # y0, x0, theta
        reg_xs = pro_reg_xs + reg[..., 4:]
    ...
    def loss(self, out_dict, img_metas):
        ...
        cls_loss /= batch_size * len(out_dict["predictions"])

        reg_xytl_loss /= batch_size * len(out_dict["predictions"])
        iou_loss /= batch_size * len(out_dict["predictions"])
