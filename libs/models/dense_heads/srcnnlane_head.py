...
class SRCNNLaneHead(nn.Module):
    def __init__(
        ...
        super(SRCNNLaneHead, self).__init__()
        self.anchor_generator = build_prior_generator(anchor_generator)
        self.init_proposal_boxes = nn.Embedding(self.anchor_generator.num_priors, fc_hidden_dim)
        self.img_w = img_w
        self.img_h = img_h
    ...
    def forward(self, x, **kwargs):
        ...
        predictions_list = []
        proposal_features = self.init_proposal_boxes.weight.unsqueeze(0).repeat(
            batch_size, 1, 1
        )  # [B, Np, Ch]
        # iterative refine
